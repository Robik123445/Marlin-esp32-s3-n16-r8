...
def run_audit() -> tuple[bool, list[str]]:
    ...
    ini_esp32 = REPO_ROOT / "ini/esp32.ini"
    board_json = REPO_ROOT / "buildroot/share/PlatformIO/boards/marlin_ESP32S3_N16R8.json"
    ...
    ok &= contains_text(ini_esp32, "[env:esp32s3_n16r8_cnc]", report)
    ok &= contains_text(ini_esp32, "board             = marlin_ESP32S3_N16R8", report)
    ok &= contains_text(ini_esp32, "platform          = espressif32@~6.7.0", report)

    board_exists = board_json.exists()
    report.append(f"{'OK' if board_exists else 'FAIL'} {board_json.relative_to(REPO_ROOT)} exists")
    ok &= board_exists
